...
export default function PackingChecklist() {
  ...
  const items = plan.items.filter(i => (showExcluded || applicable(i, plan.profile)) && (query.trim() || categoryFor(i, plan.profile) === category) && `${i.item_name} ${i.notes ?? ''}`.toLowerCase().includes(query.trim().toLowerCase()) && (filter !== 'unpacked' || !i.is_packed) && true);
  ...
  return <>
    ...
    <div className="surface mb-3 p-2.5 sm:p-4">
      <label className="block"><span className="sr-only">Search all items</span><input type="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search items…" className="field w-full" /></label>
      <div className="mt-2 grid grid-cols-2 gap-1">{[['all','All items'],['unpacked','Unchecked']].map(([value,label]) => <button key={value} aria-pressed={filter === value} onClick={() => setFilter(value)} className={`filter-chip px-2 ${filter === value ? 'selected' : ''}`}>{label}</button>)}</div>
    </div>
    <div className="mb-1 grid grid-cols-3 gap-1.5" aria-label="Categories">{['Required','Recommended','Documents'].map(cat => { const group = plan.items.filter(i => applicable(i, plan.profile) && categoryFor(i, plan.profile) === cat); return <button key={cat} aria-pressed={category === cat && !query.trim()} onClick={() => { setCategory(cat); setQuery(''); }} className={`category-button ${category === cat && !query.trim() ? 'selected' : ''}`}><span>{cat}</span><span className="block mt-1 text-xs opacity-70">{group.filter(i => i.is_packed).length} / {group.length}</span></button>; })}</div>
    <div className="mb-1 flex items-center justify-between gap-2 text-xs text-slate-500"><span role="status">{items.length} {query ? 'matching' : ''} items</span><label className="flex min-h-11 items-center gap-2"><input type="checkbox" checked={showExcluded} onChange={e => setShowExcluded(e.target.checked)} className="h-4 w-4" /> Show excluded</label></div>

    <div className="surface overflow-hidden divide-y divide-slate-100">
      {items.length === 0 && <div className="p-10 text-center"><h3 className="font-semibold">No items to show.</h3><p className="mt-2 text-sm text-slate-500">Try another category or clear your filters.</p><button className="filter-chip mt-4" onClick={() => { setQuery(''); setFilter('all'); }}>Clear filters</button></div>}
