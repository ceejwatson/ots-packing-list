...
export default function PackingChecklist() {
  ...
  return <>
    ...
    <div className="surface mb-3 p-2.5 sm:p-4">
      ...
      <div className="mt-2 grid grid-cols-3 gap-1">{[['all','All items'],['unpacked','Unpacked'],['shopping','To buy']].map(([value,label]) => <button key={value} aria-pressed={filter === value} onClick={() => setFilter(value)} className={`filter-chip px-2 ${filter === value ? 'selected' : ''}`}>{label}</button>)}</div>
    </div>
