  ...
  return <>
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl print:hidden">
      <div className="mx-auto flex h-[60px] sm:h-20 max-w-5xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-600" aria-label="Required items complete">{ready ? `${progress.percent}%` : '—'} <span className="hidden md:inline">ready</span></span>
      </div>
    </header>
