...
import { applicable, categoryFor, deadline, readiness, validDate, type Profile } from '@/lib/progress';
...
export default function PreparationOverview() {
  ...
  const profile = (patch: Partial<Profile>) => update(p => ({ ...p, profile: { ...p.profile, ...patch } }));
  return <>
    ...
    <section className="surface mt-6 p-5 sm:p-6"><h2 className="text-lg font-semibold text-slate-900">Your plan</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700">Class start date<input disabled={!ready} type="date" value={plan.profile.classDate} onChange={e => { if (validDate(e.target.value)) profile({ classDate: e.target.value }); }} className="field mt-2 w-full" /></label><label className="text-sm font-medium text-slate-700">Personal items<select disabled={!ready} value={plan.profile.section} onChange={e => profile({ section: e.target.value as Profile['section'] })} className="field mt-2 w-full"><option value="All">Show all personal items</option><option value="Womens">Women&apos;s items</option><option value="Mens">Men&apos;s items</option></select></label></div><p className="mt-3 text-xs leading-5 text-slate-500">Your date adjusts cold-weather requirements for October–May. Use “Not applicable” on individual items to refine your list.</p></section>
    <section className="surface mt-6 p-5 sm:p-6"><div className="flex flex-wrap justify-between gap-2"><h2 className="text-lg font-semibold text-slate-900">Before you arrive</h2><span className="text-xs text-slate-500">{plan.profile.classDate ? 'Dates based on your class start' : 'Add a class date for exact dates'}</span></div><ol className="mt-5 space-y-0">{deadlines.map((d, index) => <li key={d.days} className="relative flex gap-4 pb-6 last:pb-0"><div className="flex flex-col items-center"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white">{index+1}</span>{index < deadlines.length-1 && <span className="mt-2 h-full w-px bg-slate-200" />}</div><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-800">{deadline(plan.profile.classDate, d.days) ?? (d.days ? `${d.days} days before` : 'Class start')}</p><h3 className="mt-1 text-sm font-semibold text-slate-900">{d.title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{d.detail}</p></div></li>)}</ol></section>
