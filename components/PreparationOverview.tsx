...
export default function PreparationOverview() {
  const { plan, ready, error } = usePlan(); const stats = readiness(plan);
  return <>
    ...
    <section className="hero-panel"><p className="eyebrow text-blue-200">REQUIRED GEAR & DOCUMENTS</p><div className="relative z-10 mt-4 flex items-end justify-between"><div><p className="font-display text-6xl font-semibold">{ready ? stats.percent : '—'}<span className="text-3xl text-blue-200">%</span></p><p className="mt-2 text-sm text-slate-300">{stats.remaining} essentials left to check off</p></div><Link href="/" className="rounded-lg bg-white px-4 py-3 text-sm font-semibold text-slate-900">Open checklist →</Link></div><p className="relative mt-5 text-xs text-slate-300">Optional and excluded items don&apos;t affect your readiness score.</p></section>
