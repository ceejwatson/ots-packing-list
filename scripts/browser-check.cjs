...
(async () => {
  ...
  await page.getByRole('heading', { name: 'Before you arrive', exact: true }).waitFor();
  assert.equal(await page.getByText('A clear path to ready.', { exact: true }).count(), 0);
  assert.equal(await page.getByText('REQUIRED GEAR & DOCUMENTS', { exact: true }).count(), 0);
