...
(async () => {
  ...
  await page.getByLabel('Class start date').fill('2026-10-01');
  await page.getByLabel('Personal items').selectOption('Mens');
  await page.getByText('Sep 17, 2026', { exact: true }).waitFor();
  const downloadPromise = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Export backup', exact: false }).click();
  const download = await downloadPromise; await download.saveAs('test-backup.json');
  ...
  await page.locator('input[type=file]').setInputFiles('test-backup.json');
  await page.getByRole('button', { name: 'Restore backup', exact: true }).click();
  assert.ok((await page.evaluate(() => JSON.parse(localStorage.getItem('ots-preparation-v5')))).items.some(i => i.is_packed));
  await page.evaluate(() => window.scrollTo(0, 0));
  ...
  console.log('PASS: mobile navigation, no horizontal overflow, packing/reload, filters, exclusions, class dates, export/reset/restore, desktop navigation, fail-closed API, storage failure warning, failed-image shopping fallback, no page errors.');
  await browser.close();
